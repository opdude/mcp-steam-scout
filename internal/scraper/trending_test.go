...
func TestGetTrending_Epic(t *testing.T) {
	s, srv := newTestScraper(t, map[string]http.HandlerFunc{
		"/search/results/": func(w http.ResponseWriter, r *http.Request) {
			_ = json.NewEncoder(w).Encode(map[string]any{"items": []map[string]any{}})
		},
		"/graphql": func(w http.ResponseWriter, r *http.Request) {
			_ = json.NewEncoder(w).Encode(epicTrendingResponse{
				Data: struct {
					Catalog struct {
						SearchStore struct {
							Elements []struct {
					...
					Catalog: struct {
						...
						SearchStore: struct {
							...
							Elements: []struct {
								...
								{Title: "Epic Game A", ID: "uuid-1", ProductSlug: "epic-game-a", OfferType: "BASE_GAME"},
								{Title: "Epic Game B", ID: "uuid-2", ProductSlug: "epic-game-b", OfferType: "BASE_GAME"},
								{Title: "Epic DLC", ID: "uuid-3", ProductSlug: "epic-dlc", OfferType: "ADD_ON"},
							},
						},
					},
				},
	...
	if epicCount != 2 {
		t.Errorf("expected 2 epic games (no DLC), got %d", epicCount)
	}
}
